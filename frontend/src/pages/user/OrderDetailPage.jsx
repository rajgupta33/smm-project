    ...
    return (
        <div className="min-h-screen bg-surface-sunken text-ink">
            ...
            <div className="max-w-4xl mx-auto px-4 py-8">
                <button onClick={() => navigate(-1)} className="flex items-center text-ink-muted hover:text-white mb-6">
                ...
                {loading ? (
                    ...
                    <div className="text-state-danger text-center py-10">{error}</div>
                ) : (
                    <>
                        <div className="bg-surface rounded-lg p-6 mb-8">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                                ...
                                <div>
                                    <p className="text-ink-muted text-sm">Quantity</p>
                                    <p className="font-semibold">{data.order.quantity}</p>
                                </div>
                                <div>
                                    <p className="text-ink-muted text-sm">Amount</p>
                        ...
                        <div className="relative pl-6 border-l border-line">
                            {data.events.map((event, idx) => (
                                <div key={event._id || idx} className="mb-8 relative">
                                    <div className="absolute -left-10 bg-surface-sunken p-2 rounded-full text-blue-400">
                                        {getEventIcon(event.eventType)}
                                    </div>
                                    <div className="bg-surface p-4 rounded-lg ml-2">
                                        ...
                                        {event.metadata && (
                                            <pre className="text-sm text-ink-soft bg-surface p-2 rounded overflow-x-auto">
                            ...
                            {data.events.length === 0 && (
                                <p className="text-ink-muted">No activity recorded yet.</p>
                            )}
                        </div>
                    </>
                )}
