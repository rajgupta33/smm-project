    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <main className="max-w-7xl mx-auto p-6 space-y-8">
        <div>
          ...
          <p className="text-gray-400 mt-2">Configure server-side provider connections and explicit primary/fallback priority.</p>
        </div>

        <form onSubmit={createProvider} className="bg-gray-900 border border-gray-700 rounded-xl p-5 space-y-4">
            ...
            {[['code', 'Code'], ['name', 'Name'], ['apiBaseUrl', 'API base URL'], ['credentialReference', 'Credential environment reference']].map(([name, label]) => (
              <label key={name} className="text-sm text-gray-300">
                ...
                <input
                  ...
                  required
                  className="mt-1 w-full rounded bg-gray-800 border border-gray-600 p-2 text-white"
                />
              </label>
            ))}
            <label className="text-sm text-gray-300">Priority
              <input type="number" value={providerForm.priority} onChange={(event) => setProviderForm((current) => ({ ...current, priority: event.target.value }))} className="mt-1 w-full rounded bg-gray-800 border border-gray-600 p-2" />
            </label>
            <label className="text-sm text-gray-300">Timeout (ms)
              <input type="number" min="1" value={providerForm.timeoutMs} onChange={(event) => setProviderForm((current) => ({ ...current, timeoutMs: event.target.value }))} className="mt-1 w-full rounded bg-gray-800 border border-gray-600 p-2" />
            </label>
          </div>
          ...
          <button className="rounded bg-purple-600 hover:bg-purple-700 px-4 py-2 font-semibold">Create provider</button>
        </form>
        ...
        <section className="bg-gray-900 border border-gray-700 rounded-xl p-5">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {providers.map((provider) => (
              <article key={provider._id} className="rounded border border-gray-700 bg-gray-800 p-4">
                <div className="flex justify-between gap-4">
                  <div><h3 className="font-semibold">{provider.name}</h3><p className="text-sm text-gray-400">{provider.code} · {provider.healthStatus}</p></div>
                  <span className={provider.enabled ? 'text-green-400' : 'text-red-400'}>{provider.enabled ? 'Enabled' : 'Disabled'}</span>
                </div>
                <p className="text-xs text-gray-500 mt-2 break-all">{provider.apiBaseUrl}</p>
        ...
        <section className="bg-gray-900 border border-gray-700 rounded-xl p-5 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-gray-400">Review provider-authored changes before updating internal offers.</p>
            </div>
            <button type="button" onClick={load} className="rounded border border-gray-600 px-3 py-1.5 text-sm">Refresh</button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left min-w-[760px]">
              <thead><tr className="border-b border-gray-700 text-gray-400"><th className="p-2">Provider</th><th className="p-2">Run status</th><th className="p-2">Changes</th><th className="p-2">Completed</th><th className="p-2">Action</th></tr></thead>
              <tbody>{syncRuns.map((run) => {
                const provider = providers.find((item) => String(item._id) === String(run.providerId));
                return (
                  <tr key={run._id} className="border-b border-gray-800">
                    <td className="p-2">{provider?.name || 'Unknown provider'}</td>
                    <td className="p-2"><div>{run.status}</div><div className="text-xs text-gray-500">{run.applicationStatus || 'PENDING'}</div></td>
                    <td className="p-2 text-sm">{run.counts?.new || 0} new · {run.counts?.changed || 0} changed · {run.counts?.missing || 0} missing</td>
                    <td className="p-2 text-sm text-gray-400">{formatDate(run.completedAt)}</td>
                    <td className="p-2">
            ...
            {!syncRuns.length && !loading && <p className="py-4 text-gray-400">No synchronization reports yet.</p>}
          </div>

          {selectedRun && (
            <div className="rounded-xl border border-blue-700 bg-gray-950 p-5 space-y-5" aria-label="Synchronization report review">
              <div className="flex justify-between gap-3">
                <div><h3 className="text-lg font-semibold">Review report</h3><p className="text-xs text-gray-500">Generated {formatDate(selectedRun.report?.generatedAt)}</p></div>
                <button type="button" onClick={() => setSelectedRun(null)} className="text-gray-400">Close</button>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">{(selectedRun.report?.new || []).map((offer) => (
                  <div key={offer.providerServiceId} className="grid md:grid-cols-[1fr_240px] gap-3 rounded border border-gray-800 p-3">
                    <div><div className="font-medium">{offer.providerNameSnapshot}</div><div className="text-xs text-gray-500">Provider ID {offer.providerServiceId} · ₹{(offer.costRateMinor / 100).toFixed(2)} per {offer.pricingUnit} · {offer.min}–{offer.max}</div></div>
                    <label className="text-xs text-gray-400">Optional catalogue mapping
                      <select aria-label={`Catalogue mapping for ${offer.providerNameSnapshot}`} value={syncMappings[offer.providerServiceId] || ''} onChange={(event) => setSyncMappings((current) => ({ ...current, [offer.providerServiceId]: event.target.value }))} className="mt-1 w-full rounded bg-gray-800 border border-gray-600 p-2 text-sm text-white">
              ...
              <div>
                ...
                <div className="space-y-2">{(selectedRun.report?.changed || []).map((change) => {
                  ...
                  return <div key={change.providerServiceId} className={`rounded border p-3 text-sm ${extreme ? 'border-amber-600' : 'border-gray-800'}`}><div className="font-medium">Provider service {change.providerServiceId}</div><div className="text-gray-400">Fields: {change.fields.join(', ')}</div>{priceLabel && <div className={extreme ? 'text-amber-300 font-semibold' : 'text-gray-300'}>{priceLabel}{extreme ? ' · price review required' : ''}</div>}</div>;
                })}</div>
              </div>
              ...
              {reviewedExistingOffers.length > 0 && (
                <div>
                  ...
                  <p className="text-xs text-gray-500 mb-2">You may map or remap an offer observed in this report. Leaving a selection unchanged preserves its current mapping.</p>
                  <div className="grid md:grid-cols-2 gap-2">{reviewedExistingOffers.map((offer) => (
                    <label key={offer.providerServiceId} className="rounded border border-gray-800 p-3 text-xs text-gray-400">
                      {offer.providerNameSnapshot} ({offer.providerServiceId})
                      <select aria-label={`Existing catalogue mapping for ${offer.providerServiceId}`} value={syncMappings[offer.providerServiceId] ?? String(offer.catalogueServiceId || '')} onChange={(event) => setSyncMappings((current) => ({ ...current, [offer.providerServiceId]: event.target.value }))} className="mt-1 w-full rounded bg-gray-800 border border-gray-600 p-2 text-sm text-white">
              ...
              <div>
                ...
                <div className="space-y-1 text-sm text-gray-300">{(selectedRun.report?.missing || []).map((offer) => <div key={offer.providerServiceId}>{offer.providerServiceId}: {offer.currentAvailability} → {offer.proposedAvailability} ({offer.consecutiveMissingSyncs} consecutive reports)</div>)}</div>
              </div>

              <div className="text-sm text-red-300">Invalid rows skipped: {selectedRun.report?.invalid?.length || 0}</div>
              ...
              ) : <p className="text-green-400">Applied {formatDate(selectedRun.appliedAt)}</p>}
            </div>
          )}
        </section>

        <section className="bg-gray-900 border border-gray-700 rounded-xl p-5 overflow-x-auto">
          <h2 className="text-xl font-semibold mb-4">Manual priority by catalogue service</h2>
          {loading ? <p className="text-gray-400">Loading…</p> : (
            <table className="w-full text-left min-w-[800px]">
              <thead><tr className="border-b border-gray-700 text-gray-400"><th className="p-2">Service</th><th className="p-2">Primary</th><th className="p-2">Fallback</th><th className="p-2">Action</th></tr></thead>
              <tbody>{catalogue.filter((service) => service.fulfilmentType === 'PROVIDER').map((service) => {
                ...
                return (
                  <tr key={service._id} className="border-b border-gray-800">
                    <td className="p-2"><div className="font-medium">{service.displayName}</div><div className="text-xs text-gray-500">{service.min}–{service.max}</div></td>
                    <td className="p-2"><select aria-label={`Primary provider for ${service.displayName}`} value={draft.primaryProviderId || ''} onChange={(event) => setRoutingDrafts((current) => ({ ...current, [service._id]: { ...draft, primaryProviderId: event.target.value } }))} className="w-full rounded bg-gray-800 border border-gray-600 p-2"><option value="">Select primary</option>{eligible.map((provider) => <option key={provider._id} value={provider._id}>{provider.name}</option>)}</select></td>
                    <td className="p-2"><select aria-label={`Fallback provider for ${service.displayName}`} value={draft.fallbackProviderId || ''} onChange={(event) => setRoutingDrafts((current) => ({ ...current, [service._id]: { ...draft, fallbackProviderId: event.target.value } }))} className="w-full rounded bg-gray-800 border border-gray-600 p-2"><option value="">No fallback</option>{eligible.map((provider) => <option key={provider._id} value={provider._id}>{provider.name}</option>)}</select></td>
                    <td className="p-2"><button onClick={() => saveRouting(service)} className="rounded bg-purple-600 hover:bg-purple-700 px-3 py-2">Save</button></td>
                  </tr>
                );
              })}</tbody>
            </table>
