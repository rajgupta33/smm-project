...
export default function TicketsPage() {
  ...
  return <><ResponsiveNavbar /><main className="min-h-screen bg-gradient-to-br from-black to-purple-950 p-5 text-white">
    <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[400px_1fr]">
      <section><div className="flex items-center justify-between gap-3"><h1 className="text-3xl font-bold text-purple-300">Support tickets</h1>
        <select aria-label="Filter tickets" value={status} onChange={(event) => setStatus(event.target.value)}
          className="rounded border border-purple-700 bg-black p-2">{statuses.map((value) => <option key={value} value={value}>{value || 'ALL'}</option>)}</select></div>
      ...
      <section className="rounded-2xl border border-purple-800 bg-black/60 p-6">
        ...
        {!detail ? <p>Select a ticket.</p> : <><h2 className="text-2xl font-bold">{detail.ticket.publicTicketId}</h2>
          <p className="text-purple-200">{detail.ticket.category} · {detail.ticket.status} · {detail.ticket.priority}</p>
          <div className="my-4 flex flex-wrap gap-2">
            ...
            {['WAITING_FOR_CUSTOMER', 'RESOLVED', 'CLOSED'].map((value) => <button key={value} onClick={() => update({ status: value })}
              className="rounded bg-purple-700 px-3 py-2">{value}</button>)}
          </div>
          <div className="space-y-3">{detail.messages.map((message) => <article key={message.id}
            className={`rounded p-4 ${message.internalOnly ? 'border border-amber-700 bg-amber-950/50' : 'bg-gray-800'}`}>
          ...
          {detail.ticket.status !== 'CLOSED' && <form onSubmit={send} className="mt-5 space-y-3">
            <textarea aria-label="Admin reply" required maxLength={4000} value={reply} onChange={(event) => setReply(event.target.value)}
              className="min-h-24 w-full rounded border border-purple-700 bg-black p-3" />
            ...
            <button className="rounded bg-purple-600 px-5 py-3 font-semibold">Send</button>
          </form>}
        </>}
      </section>
    </div>
